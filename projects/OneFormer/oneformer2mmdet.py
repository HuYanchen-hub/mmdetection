...
def convert_pixel_decoder(model_key, model_weight, state_dict, converted_names):
    ...
    new_key = new_key.replace('transformer.level_embed', 'level_encoding.weight')
    new_key = new_key.replace('mask_features', 'mask_feature')
    new_key = new_key.replace('adapter_1.weight', 'lateral_convs.0.conv.weight')
...
def convert_class_transformer(model_key, model_weight, state_dict, converted_names):
...
def convert_transformer_decoder(model_key, model_weight, state_dict, converted_names):
    if 'transformer_self_attention_layers' in model_key:
        new_key = model_key.replace('sem_seg_head.predictor.transformer_self_attention_layers', 'panoptic_head.transformer_decoder.layers')
        new_key = new_key.replace('self_attn', 'self_attn.attn')
        new_key = new_key.replace('norm', 'norms.1') # mmdet中Decoder先放进来的是交叉注意力，与原始OneFormer相反
    elif 'transformer_cross_attention_layers' in model_key:
        new_key = model_key.replace('sem_seg_head.predictor.transformer_cross_attention_layers', 'panoptic_head.transformer_decoder.layers')
        ...
        new_key = model_key.replace('sem_seg_head.predictor.transformer_ffn_layers', 'panoptic_head.transformer_decoder.layers')
        new_key = new_key.replace('linear1', 'ffn.layers.0.0')
        new_key = new_key.replace('linear2', 'ffn.layers.1')
        ...
        if 'weight' in model_key:
            state_dict['panoptic_head.transformer_decoder.post_norm.weight'] = model_weight
        elif 'bias' in model_key:
            state_dict['panoptic_head.transformer_decoder.post_norm.bias'] = model_weight
        ...
        new_key = model_key.replace('sem_seg_head.predictor.class_embed', 'panoptic_head.cls_embed')
    state_dict[new_key] = model_weight
    converted_names.add(model_key)
    print(f'Convert {model_key} to {new_key}')    
...
def convert(src, dst):
    ...
    for key, weight in blobs.items():
        if 'backbone' in key:
            ...
            convert_transformer_decoder(key, weight, state_dict, converted_names)
